...
// QA button events
const qaButtons = document.querySelectorAll('.qas button');
const showAns = (e) => {
  const icon = e.target;
  const ans = icon.closest('.qa-inner').children[1];
  icon.classList.toggle('qa-reverse');
  ans.classList.toggle('qa-toggle');
}

qaButtons.forEach((qa) => qa.addEventListener('click', showAns));